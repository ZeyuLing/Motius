...
def test_smpl_loader_accepts_published_smplh_npz_layout(tmp_path: Path) -> None:
    model_path = tmp_path / "body_models" / "smplh" / "neutral" / "model.npz"
    model_path.parent.mkdir(parents=True)
    parents = np.asarray(SMPL22_PARENTS, dtype=np.int64)
    rest = np.zeros((22, 3), dtype=np.float32)
    rest[:, 1] = np.arange(22, dtype=np.float32) * 0.05
    np.savez(
        model_path,
        J_regressor=np.eye(22, dtype=np.float32),
        v_template=rest,
        shapedirs=np.zeros((22, 3, 10), dtype=np.float32),
        kintree_table=np.stack((parents, np.arange(22, dtype=np.int64))),
    )

    assert _resolve_smplh_npz(tmp_path / "body_models", "neutral") == model_path
    model, loaded_rest, loaded_parents = load_smpl_rest(
        tmp_path / "body_models", "cpu"
    )
    np.testing.assert_allclose(loaded_rest, rest, atol=1e-6)
    np.testing.assert_array_equal(loaded_parents, parents)

    translation = torch.tensor([[0.2, 0.3, -0.4]], dtype=torch.float32)
    output = model(
    ...
    np.testing.assert_allclose(
        output.joints.detach().numpy()[0],
        rest + translation.numpy()[0],
        atol=1e-6,
    )
