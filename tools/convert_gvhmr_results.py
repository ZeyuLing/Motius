...
def main() -> None:
    ...
    if args.runtime_revision != OFFICIAL_RUNTIME_REVISION:
        raise RuntimeError(
            f"Runtime revision mismatch: {args.runtime_revision} != "
            f"{OFFICIAL_RUNTIME_REVISION}"
        )
    device = torch.device(
        "cuda"
    ...
    from motius.models.gvhmr.vendor import hmr4d as _vendored_hmr4d
