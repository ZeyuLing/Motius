...
def load_motion135(path: Path, *, max_frames: int | None = None) -> np.ndarray:
    ...
    if isinstance(loaded, np.lib.npyio.NpzFile):
        try:
            if "motion_135" in loaded.files:
            ...
            elif "motion_272" in loaded.files:
                from motius.motion.representation.motion272 import (
                    motion272_to_motion135,
                )

                value = motion272_to_motion135(
                    np.asarray(loaded["motion_272"], dtype=np.float32)
                )
                ...
                global_orient = np.asarray(
                    loaded["global_orient"], dtype=np.float32
                ).reshape(-1, 1, 3)
                body_pose = np.asarray(
                    loaded["body_pose"], dtype=np.float32
                ).reshape(len(global_orient), -1, 3)
                translation_key = "transl" if "transl" in loaded.files else "trans"
                translation = np.asarray(
                    loaded[translation_key], dtype=np.float32
                ).reshape(-1, 3)
                ...
                value = np.concatenate(
                    (translation[: len(axis_angle)], rotation6d), axis=1
                )
            else:
                raise KeyError(
                    f"motion_135 or SMPL pose parameters not found in {path}: "
                    f"{loaded.files}"
                )
        finally:
            loaded.close()
