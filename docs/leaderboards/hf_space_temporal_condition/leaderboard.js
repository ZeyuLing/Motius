...
function controlRows(data) {
  ...
  return data.settings.flatMap((setting) => {
    ...
    const methods = setting.methods.filter((entry) => !entry.is_reference).map((entry) => ({
      method: entry.method,
      settingId: setting.id,
      settingLabel: label,
      settingGroupId,
...
async function initialize() {
  ...
  state.rowsByProtocol.control = controlRows(data);
  state.rowsByProtocol.tp2m = Array.from(data.tp2m_rows);
  document.querySelectorAll("[data-protocol]").forEach((button) => {
    button.addEventListener("click", () => {
