---
title: Temporal Motion Completion · HumanML3D
emoji: ⏱️
colorFrom: green
colorTo: indigo
sdk: static
...
reused from the matching Text-to-Motion · HumanML3D benchmark rather than
recomputed. For
...
The quantitative table includes all eight 4,012-case ProjFlow runs. ProjFlow's
native Motius artifact and task APIs are documented in its
[Model Card](../../model_zoo/projflow.md). Its all-case mesh pack has not been
added to the qualitative explorer, so the viewer does not present a partial or
mixed-protocol ProjFlow row.
...
maintained separately on the Sequential Text-to-Motion · BABEL benchmark.
