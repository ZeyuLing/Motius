...
function renderRows(data){
  ...
  const generated=rows.filter(row=>!row.reference).sort((a,b)=>a.fid_utmr-b.fid_utmr);
  const displayed=[...rows.filter(row=>row.reference),...generated];
  document.getElementById("method-count").textContent=generated.length;
  document.getElementById("rows").innerHTML=displayed.map((row)=>{
    const primaryRank=row.reference?null:ranks.fid_utmr.get(row.method);
