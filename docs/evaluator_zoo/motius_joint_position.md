...
This evaluator is trainable entirely through Motius. The public
[configuration](../../configs/tmr/train_tmr_smpl22.py) builds `TMRBundle`,
`TMRTrainer`, and `TMRTextMotionDataset` without importing the original TMR
repository.

Materialize canonical SMPL-22 joints66 motions, captions, splits, sentence and
token embeddings, and training statistics using the layout in the
[TMR data-format guide](../training/prism_tmr_hymotion_t2m.md). The exact
released checkpoint used full HYMotion Data SFT and the full single-person
MotionHub training union; HYMotion Data is not currently distributed through
the public Dataset Hub.

```bash
MOTIUS_DATA_ROOT=/path/to/tmr_joints66 \
...
| Objective | Reconstruction `1.0`, contrastive `0.1`, latent `1e-5`, and KL `1e-5` |
| Retrieval temperature | `0.1`, self-similarity threshold `0.8` |
| Schedule | 300 epochs, gradient norm clipped to `1.0` |
| Checkpoints | Every epoch, latest state saved, five historical checkpoints retained |
| Outputs | `outputs/training/tmr_smpl22` |

Use `--load-from CHECKPOINT --load-scope full` for an explicit full-state
resume. Dataset preparation, multi-node launch, and output rules are collected
in the [Training Hub](../training/README.md).
