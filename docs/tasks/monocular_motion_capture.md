...
- GVHMR: source-pinned Motius runtime with exact official checkpoints, SMPL
  camera and world output.
- PromptHMR-Video: restricted official-runtime adapter. The upstream license
  prevents Motius from vendoring or redistributing a complete runtime, so this
  method is excluded from the release-complete Task Index. World metrics remain
  unavailable unless the upstream output explicitly contains a world
  trajectory.
- GEM-SMPL: source-pinned Motius runtime with a complete HF artifact and
  native SMPL-24 joints/mesh. The paper/repository was originally named GENMO.
- GEM-X: source-pinned Motius runtime with a complete HF artifact and native
  SOMA-77 outputs. The pinned demo uses an identity camera trajectory when no
  external visual odometry is supplied, so current verified results are
  camera-space only.
