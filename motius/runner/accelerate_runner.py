...
from motius.utils.logger import get_logger, add_file_handler
from motius.utils.checkpoint_utils import find_latest_checkpoint
from motius.utils.env import collect_env_info
from motius.runner.loops import EpochBasedLoop, IterBasedLoop
...
def _cuda_lib_warmup_enabled() -> bool:
    return os.environ.get('MOTIUS_CUDA_LIB_WARMUP', '0').lower() in {'1', 'true', 'yes', 'on'}
...
def _warmup_cuda_libraries(accelerator: Accelerator) -> None:
    ...
    lock_dir = os.environ.get('MOTIUS_CUDA_LIB_WARMUP_LOCK_DIR', '/tmp')
    os.makedirs(lock_dir, exist_ok=True)
    lock_path = os.path.join(lock_dir, 'motius_cuda_lib_warmup.lock')
...
class AccelerateRunner:
    """
    Main training runner that integrates Accelerate with the motius framework.
    ...
    def from_cfg(cls, cfg) -> 'AccelerateRunner':
        ...
        if pg_timeout_sec is None:
            pg_timeout_sec = os.environ.get('MOTIUS_PROCESS_GROUP_TIMEOUT_SEC')
    ...
    def _pre_prepare_load(bundle, cfg, work_dir, accelerator):
        ...
        **Memory optimisation**: In distributed mode the checkpoint is
        ...
        """
        from motius.utils.checkpoint_utils import (
    ...
    def _build_single_optimizer(opt_cfg: dict, params) -> torch.optim.Optimizer:
        ...
        if hasattr(optim, opt_type):
            ...
            try:
                ...
                from motius.registry import _import_hf_class
    ...
    def _build_hooks(hooks_cfg: dict) -> list:
        ...
        from motius.registry import HOOKS
        hooks = []
    ...
    def _build_visualizers(vis_cfg) -> list:
        ...
        if vis_cfg is None:
            return []
        from motius.registry import VISUALIZERS
    ...
    def _state_dict_to_save(self) -> Dict[str, dict]:
        ...
        state_dict = {'__motius_meta__': self.bundle.checkpoint_metadata()}
            ...
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                try:
                    with self.accelerator.accumulate(*[
                        ...
                        debug_first_steps = int(os.environ.get('MOTIUS_DEBUG_FIRST_STEPS', '2'))
                        debug_first_step = (
                            os.environ.get('MOTIUS_DEBUG_FIRST_STEP') == '1'
                            and batch_idx < debug_first_steps
                        )
                        sync_after_optimizer_step = (
                            os.environ.get('MOTIUS_SYNC_AFTER_OPTIMIZER_STEP') == '1'
                        )
                        sync_after_backward = (
                            os.environ.get('MOTIUS_SYNC_AFTER_BACKWARD') == '1'
                        )
                        if debug_first_step:
                            logger.info(
