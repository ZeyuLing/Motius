...
def resume_training(config):
    if config.get("checkpoint", None) is not None:
        last_existing_checkpoint = os.path.abspath(config.checkpoint)
        if not os.path.isfile(last_existing_checkpoint):
            raise FileNotFoundError(
                f"Explicit SONIC checkpoint does not exist: {last_existing_checkpoint}"
            )
    else:
        # Hydra always resolves experiment_dir with a fresh timestamp. Search
        # sibling runs with the same experiment name instead of treating the new
        # directory as an existing run.
        experiment_dir = os.path.abspath(config.experiment_dir)
        current_checkpoint = os.path.join(experiment_dir, "last.pt")
        experiment_dir_base = re.sub(
            r"-(?:\d{8}_\d{6}|\d{14})$", "", experiment_dir
        )
        checkpoints = glob.glob(os.path.join(f"{experiment_dir_base}-*", "last.pt"))
        if os.path.isfile(current_checkpoint):
            checkpoints.append(current_checkpoint)
        if not checkpoints:
            print(f"No checkpoint found matching {experiment_dir_base}-*/last.pt, starting fresh")
            return
        last_existing_checkpoint = max(checkpoints, key=os.path.getmtime)
