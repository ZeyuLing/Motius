...
def _is_complete_wan_runtime(path: Path) -> bool:
    return (
        (path / "config.json").is_file()
        and (path / "models_t5_umt5-xxl-enc-bf16.pth").is_file()
        and (path / "google" / "umt5-xxl" / "tokenizer_config.json").is_file()
    )
...
def _resolve_wan_dir(
    ...
    if wan_dir:
        ...
        if path.is_absolute():
            ...
            if _is_complete_wan_runtime(resolved):
                return resolved
            raise FileNotFoundError(f"Incomplete ViMoGen Wan runtime: {resolved}")
        for base in (artifact, _REPO_ROOT):
            candidate = (base / path).resolve()
            if _is_complete_wan_runtime(candidate):
                return candidate
        raise FileNotFoundError(f"Incomplete ViMoGen Wan runtime: {artifact / path}")
    ...
    raise FileNotFoundError(
        "The ViMoGen artifact is incomplete: missing the bundled Wan config "
        "and UMT5 text encoder under 'wan/'. Pass wan_dir explicitly only for "
        "raw checkpoint conversion."
    )
...
class ViMoGenBundle(ModelBundle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        context_null_path = _artifact_path(
            context_null_path
            or index.get("context_null_path")
            or "context_null_padded.pth",
            artifact,
        )
        ...
        if not context_null_path.exists():
            raise FileNotFoundError(
                f"ViMoGen null text context not found: {context_null_path}"
            )
    ...
    def save_pretrained(self, save_directory: str, **kwargs):
        ...
        include_wan = bool(kwargs.pop("include_wan", True))
        shutil.copy2(self.checkpoint, save_dir / "model.pt")
        stats_dir = save_dir / "assets" / "meta"
        stats_dir.mkdir(parents=True, exist_ok=True)
        np.save(stats_dir / "mean.npy", self.mean.detach().cpu().numpy())
        np.save(stats_dir / "std.npy", self.std.detach().cpu().numpy())
        shutil.copy2(self.context_null_path, save_dir / "context_null_padded.pth")
        if include_wan:
            wan_dir = save_dir / "wan"
            (wan_dir / "google" / "umt5-xxl").mkdir(parents=True, exist_ok=True)
            shutil.copy2(self.wan_dir / "config.json", wan_dir / "config.json")
            shutil.copy2(
                self.wan_dir / "models_t5_umt5-xxl-enc-bf16.pth",
                wan_dir / "models_t5_umt5-xxl-enc-bf16.pth",
            )
            for source in (self.wan_dir / "google" / "umt5-xxl").iterdir():
                if source.is_file():
                    shutil.copy2(
                        source,
                        wan_dir / "google" / "umt5-xxl" / source.name,
                    )
        index = {
            "model_type": "vimogen",
            "checkpoint": "model.pt",
            ...
            "context_null_path": "context_null_padded.pth",
            "motion_representation": "vimogen276",
            "cfg_scale": self.cfg_scale,
            "denoising_strength": self.denoising_strength,
            "num_inference_steps": self.num_inference_steps,
            "text_encoder": "Wan2.1-T2V-1.3B UMT5-XXL",
            "wan_dir": "wan" if include_wan else None,
            "text_encoder_stored_in_artifact": include_wan,
        }
