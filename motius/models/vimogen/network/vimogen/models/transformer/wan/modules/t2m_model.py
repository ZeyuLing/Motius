...
class WanModelT2M(nn.Module):
    ...
    def __init__(self,
        ...
        if split_head:
            self.local_motion_head = Head(dim, 21*6, patch_size, eps)
            self.fusion = nn.Linear(dim + 21*6, dim)
            self.global_motion_head = Head(dim, out_dim-21*6, patch_size, eps)
        else:
    ...
    def forward(
        ...
        # head
        if self.split_head:
            local_motion = self.local_motion_head(x, e)
            x = self.fusion(torch.cat([x, local_motion], dim=-1))
            global_motion = self.global_motion_head(x, e)
            x = torch.cat([local_motion, global_motion], dim=-1)
        else:
