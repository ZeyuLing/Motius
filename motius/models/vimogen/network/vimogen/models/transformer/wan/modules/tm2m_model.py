...
def flash_attention(
    ...
    q_lens_full, k_lens_full = q_lens, k_lens
    ...
    if (version is None or version == 3) and FLASH_ATTN_3_AVAILABLE:
        ...
        q_pad = half(q_full).to(v_full.dtype)
        if q_scale is not None:
            q_pad = q_pad * q_scale
        k_pad = half(k_full).to(v_full.dtype)
        v_pad = half(v_full)
        if q_lens_full is None and k_lens_full is None:
            q_sdpa = q_pad.transpose(1, 2).to(dtype)
            k_sdpa = k_pad.transpose(1, 2).to(dtype)
            v_sdpa = v_pad.transpose(1, 2).to(dtype)
            x = torch.nn.functional.scaled_dot_product_attention(
                q_sdpa, k_sdpa, v_sdpa, is_causal=causal, dropout_p=dropout_p)
            x = x.transpose(1, 2).contiguous()
        else:
            q_lens_loop = (
                q_lens_full.to(device=q_pad.device, dtype=torch.long)
                if q_lens_full is not None
                else torch.full((b,), lq, device=q_pad.device, dtype=torch.long)
            )
            k_lens_loop = (
                k_lens_full.to(device=k_pad.device, dtype=torch.long)
                if k_lens_full is not None
                else torch.full((b,), lk, device=k_pad.device, dtype=torch.long)
            )
            x = v_pad.new_zeros((b, lq, v_pad.size(2), v_pad.size(3)))
            for idx in range(b):
                q_len_i = int(q_lens_loop[idx].item())
                k_len_i = int(k_lens_loop[idx].item())
                if q_len_i <= 0 or k_len_i <= 0:
                    continue
                qi = q_pad[idx:idx + 1, :q_len_i].transpose(1, 2).to(dtype)
                ki = k_pad[idx:idx + 1, :k_len_i].transpose(1, 2).to(dtype)
                vi = v_pad[idx:idx + 1, :k_len_i].transpose(1, 2).to(dtype)
                oi = torch.nn.functional.scaled_dot_product_attention(
                    qi, ki, vi, is_causal=causal, dropout_p=dropout_p)
                x[idx:idx + 1, :q_len_i] = oi.transpose(1, 2).contiguous().to(v_pad.dtype)
