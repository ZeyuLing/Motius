...
class MotionEncoderTransformer(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, device, num_layers=2, nhead=2):
        ...
        encoder_layers = nn.TransformerEncoderLayer(hidden_size, nhead, hidden_size * 2, dropout=0.1)
    ...
    def forward(self, inputs, m_lens):
        ...
        # Pass inputs through the embedding layer first
        input_embs = self.input_emb(inputs)

        # Add positional encoding
        pos_enc_input = self.positional_encoding(input_embs)

        # Padding mask for attention to ignore padded elements
        pad_mask = (inputs == 0).all(dim=2)

        # Transformer expects src to be of shape (seq_len, batch, features)
        transformer_out = self.transformer_encoder(pos_enc_input.permute(1, 0, 2), src_key_padding_mask=pad_mask)

        # We take only the last output for each sequence to pass to the output network
        # You may need to adjust this depending on how you want to handle sequence outputs
...
# Assuming init_weight is a function defined elsewhere that initializes weights
def init_weight(m):
