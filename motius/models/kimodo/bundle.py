...
class KIMODOBundle(ModelBundle):
    ...
    def from_config(cls, cfg: Optional[dict] = None, **kwargs):
        ...
        components = deepcopy(cfg_dict.get("components", {}))
        ...
        text_encoder_component = components.get("text_encoder", {})
        if (
            should_resolve_text_encoders
            and text_encoder_component.get("stored_in_artifact") is True
            and (not text_encoders_dir or not Path(text_encoders_dir).exists())
        ):
            raise FileNotFoundError(
                "The KIMODO artifact is incomplete: its bundled text encoder "
                f"directory is missing at {text_encoders_dir!r}."
            )
