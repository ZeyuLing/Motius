...
try:
    import clip
except ImportError as exc:  # pragma: no cover - exercised only without CLIP installed
    clip = None
    _CLIP_IMPORT_ERROR = exc

# BERT text encoder is only needed for the `trans_dec` + bert checkpoints; the
# motius bundle targets the public CLIP `trans_enc` HumanML3D model, so make it
# optional rather than dragging the BERT package into motius.
try:
    ...
    def load_bert(*_args, **_kwargs):  # pragma: no cover - bert path unused
        raise RuntimeError(
            ...
            "checkpoint is supported in motius."
        )
...
class MDM(nn.Module):
            ...
            if 'text' in self.cond_mode:
                ...
                if self.text_encoder_type == "clip":
                    ...
                    self.clip_model = self.load_and_freeze_clip(
                        clip_version,
                        weights_path=kargs.get("clip_weights_path"),
                    )
    ...
    def load_and_freeze_clip(self, clip_version, weights_path=None):
        if clip is None:
            raise RuntimeError(
                "MDM inference requires OpenAI CLIP. Install it with "
