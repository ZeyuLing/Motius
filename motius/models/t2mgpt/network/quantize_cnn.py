...
class QuantizeEMAReset(nn.Module):
    ...
    def reset_codebook(self):
        ...
        # NOTE: upstream hard-codes ``.cuda()`` here. Dropped so the module can
        # be constructed on CPU; the codebook buffer is overwritten by the
        # checkpoint on ``load_state_dict`` and follows ``.to(device)`` after.
...
class QuantizeEMA(nn.Module):
    ...
    def reset_codebook(self):
        ...
        self.code_sum = None
        self.code_count = None
        # See QuantizeEMAReset.reset_codebook: ``.cuda()`` dropped for CPU support.
        self.register_buffer('codebook', torch.zeros(self.nb_code, self.code_dim))
