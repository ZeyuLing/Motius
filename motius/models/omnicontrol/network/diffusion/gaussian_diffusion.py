...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        if dataset != 'humanml':
            raise NotImplementedError('The released OmniControl checkpoint is HumanML3D-only')
        required = {'raw_mean': raw_mean, 'raw_std': raw_std, 'mean': mean, 'std': std}
        missing = [name for name, value in required.items() if value is None]
        if missing:
            raise ValueError(f'OmniControl diffusion requires artifact statistics: {missing}')
        self.raw_mean = torch.as_tensor(raw_mean, dtype=torch.float32)
        self.raw_std = torch.as_tensor(raw_std, dtype=torch.float32)
    ...
    def calc_grad_scale(self, mask_hint):
        controlled_joint = mask_hint.any(dim=-1)
        num_keyframes = controlled_joint.sum(dim=1)
        max_keyframes = num_keyframes.max(dim=1)[0]
        scale = 20 / max_keyframes.clamp_min(1)
    ...
    def guide(self, x, t, model_kwargs=None, t_stopgrad=-10, scale=.5, n_guide_steps=10, train=False, min_variance=0.01):
        ...
        mask_hint = model_kwargs['y'].get('hint_axis_mask')
        if mask_hint is None:
            mask_hint = hint.view(hint.shape[0], hint.shape[1], n_joint, 3).sum(dim=-1, keepdim=True) != 0
        else:
            mask_hint = mask_hint.to(device=hint.device, dtype=torch.bool)
