...
from . import hmr4d as _vendored_hmr4d
...
def parse_args_to_cfg():
    ...
    parser.add_argument("--parity_trace", type=str, default=None)
    ...
    with initialize_config_module(version_base="1.3", config_module=f"hmr4d.configs"):
        ...
        if args.ckpt_path is not None:
            overrides.append(f"ckpt_path={Path(args.ckpt_path).resolve()}")
...
def capture_parity(cfg, data, parity_trace, pred=None):
    """Capture the documented official demo boundary tensors."""

    if not parity_trace or pred is None:
        return
    ...
    capture_gvhmr_trace(
        parity_trace,
        name="gvhmr-motius",
        bbox=torch.load(cfg.paths.bbx, map_location="cpu", weights_only=True),
        data=data,
        prediction=pred,
        metadata={
            "source_revision": (
            ...
            "static_camera": bool(cfg.static_cam),
            "use_dpvo": bool(cfg.use_dpvo),
        },
    )
...
def render_incam(cfg):
    ...
    smplx2smpl = torch.load(
        SOURCE_ROOT / "hmr4d/utils/body_model/smplx2smpl_sparse.pt"
    ).cuda()
...
def render_global(cfg):
    ...
    J_regressor = torch.load(
        SOURCE_ROOT / "hmr4d/utils/body_model/smpl_neutral_J_regressor.pt"
    ).cuda()
...
if __name__ == "__main__":
    ...
    capture_parity(cfg, data, args.parity_trace, pred)
    ...
    if args.render:
        ...
        elif not Path(paths.incam_global_horiz_video).exists():
            Log.warning(
                "[Merge Videos] Skipped because the ffmpeg executable is absent."
            )
