...
class DARTBundle(ModelBundle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.clip_weights_path = (
            Path(clip_weights_path).resolve() if clip_weights_path else None
        )
        ...
        if self.clip_weights_path is None or not self.clip_weights_path.is_file():
            raise FileNotFoundError(
                "DART requires artifact-local CLIP weights at clip.safetensors"
            )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs):
        ...
        if path.is_dir() and (path / _DEFAULT_DENOISER).exists():
            ...
            if not clip_weights.exists():
                raise FileNotFoundError(
                    f"The DART artifact is incomplete: missing {path / 'clip.safetensors'}"
                )
            return cls(
    ...
    def _get_dataset(self, batch_size: int):
        ...
        with artifact_clip_load(
            self.clip_weights_path,
            expected_name="ViT-B/32",
        ):
            with _dart_runtime(self.artifact_dir):
