...
def _lora_linear(in_features, out_features, *, use_lora=False, lora_rank=16):
    if use_lora:
        if lora is None:
            raise ModuleNotFoundError(
                "loralib is required when use_lora=True, but this DART "
                "checkpoint does not use LoRA in its default Transformer path."
            )
        return lora.Linear(in_features, out_features, r=lora_rank)
...
class MLP(nn.Module):
    def __init__(self, in_dim,
        ...
        in_dim_ = in_dim
        for h_dim in h_dims:
            layer = _lora_linear(in_dim_, h_dim, use_lora=use_lora, lora_rank=lora_rank)
...
class MLPBlock(nn.Module):
    def __init__(self, h_dim, out_dim, n_blocks, actfun='relu', residual=True, use_lora=False, lora_rank=16):
        ...
        self.out_fc = _lora_linear(h_dim, out_dim, use_lora=use_lora, lora_rank=lora_rank)
