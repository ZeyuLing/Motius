...
* ``self.text_encoder`` — :class:`MldTextEncoder` (frozen ``sentence-t5-large``
  SentenceTransformer, stored in the Motius artifact).
...
_TEXT_ENCODER_SUBDIR = "text_encoder/sentence-t5-large"
...
class MotionLCMBundle(ModelBundle):
    ...
    def __init__(
        ...
        * **Self-contained Motius artifact** — ``config`` plus
          ...
          ``Mean.npy`` / ``Std.npy`` and the complete text backbone, as
    ...
    def save_pretrained(self, save_directory: str, safe_serialization: bool = True, **kwargs):
        ...
        The sentence-t5-large text encoder is stored below ``text_encoder/``.
        """
        import os
        ...
        cfg = {
            ...
            "text_encoder_name": _TEXT_ENCODER_SUBDIR,
        ...
        if include_text_encoder:
            if self.text_encoder is None:
                raise RuntimeError(
                    "Cannot export a complete MotionLCM artifact without a "
                    "loaded text encoder."
                )
            self.text_encoder.text_model.save(
                str(save_dir / _TEXT_ENCODER_SUBDIR),
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs):
        ...
        explicit_text_encoder = kwargs.pop("text_encoder_name", None)
        load_text_encoder = bool(kwargs.get("load_text_encoder", True))
        bundled_text_encoder = path / str(
            meta.get("text_encoder_name", _TEXT_ENCODER_SUBDIR)
        )
        if explicit_text_encoder is not None:
            text_encoder_name = explicit_text_encoder
        elif bundled_text_encoder.is_dir():
            text_encoder_name = str(bundled_text_encoder)
        elif load_text_encoder:
            raise FileNotFoundError(
                "The MotionLCM artifact is incomplete: missing bundled "
                f"SentenceT5 text encoder at {bundled_text_encoder}."
            )
        else:
            text_encoder_name = _DEFAULT_TEXT_ENCODER
