"""MLD motion VAE (adapted from MotionLCM ``mld/.../mld_vae.py``).

Transformer VAE that encodes a ``(B, T, nfeats)`` motion (HumanML3D-263) into a
small set of latent tokens ``z`` and decodes back. MotionLCM operates entirely
in this latent space: the consistency denoiser predicts ``z`` and ``decode``
maps it to the 263-dim feature. The released HumanML3D checkpoint uses
``latent_dim=[16, 32, 256]`` (16 latent tokens, 32-dim code, 256-dim
transformer). Self-contained;.
"""
...
class MldVae(nn.Module):
    ...
    def decode(self, z: torch.Tensor, lengths: list) -> torch.Tensor:
