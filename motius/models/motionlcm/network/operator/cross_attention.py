"""Skip-connected Transformer encoder/decoder (adapted from MotionLCM).

Verbatim port of ``mld/models/operator/cross_attention.py`` (itself derived
from MLD / TEMOS). Provides ``SkipTransformerEncoder`` (the U-Net-style skip
transformer used by both the MLD VAE and the MotionLCM denoiser),
``SkipTransformerDecoder`` (VAE decoder) and the plain encoder/decoder layers.
Self-contained:.
"""
...
class SkipTransformerEncoder(nn.Module):
    ...
    def forward(self, src: torch.Tensor,
                ...
                controlnet_residuals: Optional[list] = None) -> torch.Tensor:
        x = src
        intermediate = []
        index = 0
        xs = []
