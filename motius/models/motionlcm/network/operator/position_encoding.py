"""1D position encodings (adapted from MotionLCM ``mld/models/operator``).

Verbatim port of ``PositionEmbeddingSine1D`` / ``PositionEmbeddingLearned1D``
and the ``build_position_encoding`` factory. Kept self-contained so the
MotionLCM bundle/pipeline never depend on the upstream checkout.
"""
