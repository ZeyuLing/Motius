"""MLD / MotionLCM latent denoiser (adapted from ``mld/.../mld_denoiser.py``).

Transformer that predicts the latent ``z`` given a noisy latent, a timestep and
a text embedding. For the **MotionLCM** (consistency) checkpoint
``time_cond_proj_dim`` is set (256), enabling the "distilled CFG" path where the
guidance scale is injected through ``time_embedding.cond_proj`` rather than via a
second unconditional forward pass. Self-contained;.

The ControlNet branch from the upstream file is preserved verbatim for fidelity
but is unused on the text-to-motion inference path.
"""
...
class MldDenoiser(nn.Module):
    ...
    def forward(self,
                ...
                controlnet_residuals: Optional[list] = None
