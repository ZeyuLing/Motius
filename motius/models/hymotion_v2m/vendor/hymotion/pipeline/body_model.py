...
from ..datasets.geometry import rot6d_to_rotation_matrix, rotation_matrix_to_angle_axis
...
def simple_lbs(v_template, rot_mats, joints, parents, skin_weights, skin_indices):
    ...
    dtype = skin_weights.dtype

    rot_mats = rot_mats.to(device=device, dtype=dtype)
    joints = joints.to(device=device, dtype=dtype)
    v_template = v_template.to(device=device, dtype=dtype)
    skin_weights = skin_weights.to(device=device, dtype=dtype)
    skin_indices = skin_indices.to(device=device)
...
class WoodenMesh(torch.nn.Module):
    ...
    def __init__(self, model_path="scripts/web/static/assets/dump_wooden"):
...
if __name__ == "__main__":
    ...
    model_path = "scripts/web/static/assets/dump_wooden"
