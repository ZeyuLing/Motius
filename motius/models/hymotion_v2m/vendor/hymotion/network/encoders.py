...
class FinalLayer(nn.Module):
    def __init__(self, feat_dim: int, out_dim: int, act_type: str = "gelu", zero_init=False, **kwargs):
        ...
        self.norm_final = nn.LayerNorm(feat_dim, elementwise_affine=False, eps=1e-5)
    ...
    def forward(self, x: Tensor, adapter: Tensor) -> Tensor:
        ...
        x_norm = self.norm_final(x).float()
        x = modulate(x_norm, shift=shift, scale=scale)
        dtype = self.linear.weight.dtype
...
class TimestepEmbeddingEncoder(nn.Module):
    ...
    def forward(self, t: Tensor) -> Tensor:
        emb = self.sinusodial_embedding(t, self.embedding_dim, time_factor=self.time_factor)
        x = self.blocks(emb.to(dtype=self.blocks[0].weight.dtype)).unsqueeze(1)
        return x
    ...
    def sinusodial_embedding(
        ...
        dtype, device = torch.float32, timesteps.device
        timesteps = timesteps.to(dtype=dtype) * time_factor
        half = embedding_dim // 2
        freqs = torch.exp(
            -torch.log(torch.tensor(temperature, device=device, dtype=dtype))
