...
def _make_native_prefix_constraint(
    model,
    native_motion: Mapping[str, Any],
    cond_frames: int,
):
    from motius.models.kimodo.network.constraints import FullBodyConstraintSet
    ...
    skeleton = model.skeleton.to(device)
    local = _to_tensor(
        native_motion["local_rot_mats"],
        device=device,
    )
    root = _to_tensor(
        native_motion["root_positions"],
        device=device,
    )
    while local.ndim > 4 and local.shape[0] == 1:
        local = local[0]
    while root.ndim > 2 and root.shape[0] == 1:
        root = root[0]
    if (
        isinstance(skeleton, SOMASkeleton30)
        and local.shape[-3] == skeleton.somaskel77.nbjoints
    ):
        local = skeleton.from_SOMASkeleton77(local)
    if local.ndim != 4 or local.shape[-2:] != (3, 3):
        raise ValueError(
            "native KIMODO local_rot_mats must have shape (T,J,3,3), "
            f"got {tuple(local.shape)}"
        )
    if local.shape[-3] != skeleton.nbjoints:
        raise ValueError(
            "native KIMODO joint count does not match the runtime skeleton "
            f"({local.shape[-3]} != {skeleton.nbjoints})"
        )
    if root.shape != (len(local), 3):
        raise ValueError(
            "native KIMODO root_positions must have shape "
            f"({len(local)},3), got {tuple(root.shape)}"
        )

    global_rots, positions, _ = skeleton.fk(local, root)
    k = max(1, min(int(cond_frames), len(local)))
    frame_idx = torch.arange(k, device=device, dtype=torch.long)
    constraint = FullBodyConstraintSet(
        skeleton,
        frame_indices=frame_idx,
        global_joints_positions=positions[:k],
        global_joints_rots=global_rots[:k],
        smooth_root_2d=positions[:k, skeleton.root_idx][:, [0, 2]],
        to_crop=False,
    )
    return constraint, len(local)
...
class KIMODOPipeline:
    ...
    def _constraint_device(self, device=None):
        if device is not None:
            return device
        return getattr(self.bundle.model, "device", None)
    ...
    def infer_text_to_motion(
        ...
        **kwargs,
        ...
        """Generate unconstrained motion through the canonical task API."""
        if isinstance(prompts, str):
            return self.text_to_motion(prompts, int(num_frames), **kwargs)
        prompt_list = list(prompts)
        if isinstance(num_frames, int):
            frame_list = [int(num_frames)] * len(prompt_list)
        else:
            frame_list = [int(value) for value in num_frames]
        if len(frame_list) != len(prompt_list):
            raise ValueError("num_frames must contain one value per prompt")
        return self.bundle.generate(
            prompt_list,
            frame_list,
            constraints=None,
    ...
    infer_kinematic_motion_control = constrained_motion
    infer_control = constrained_motion

    def infer_sequential_text_to_motion(
        self,
        prompts: Sequence[str],
        segment_frames: Sequence[int],
        **kwargs,
    ) -> Dict[str, Any]:
        """Generate one blended sequence from ordered text segments."""
        return self.multi_prompt(prompts, segment_frames, **kwargs)

    infer_sequential_t2m = infer_sequential_text_to_motion
    ...
    def infer_temporal_motion_completion(
        self,
        prompts,
        reference_motions,
        condition_frames: int,
        **kwargs,
        ...
        """Complete motion from either SMPL ``motion_135`` or native KIMODO output."""
        if isinstance(prompts, str):
            prompts = [prompts]
        if isinstance(reference_motions, Mapping):
            reference_motions = [reference_motions]
        prompt_list = list(prompts)
        reference_list = list(reference_motions)
        if len(prompt_list) != len(reference_list):
            raise ValueError("prompts and reference_motions must have equal length")
        if not reference_list or not all(
            isinstance(value, Mapping) for value in reference_list
        ):
            return self.infer_tp2m(
                prompt_list,
                reference_list,
                condition_frames,
                **kwargs,
        ...
        model = self.bundle.model
        outputs = []
        for prompt, reference in zip(prompt_list, reference_list):
            constraint, num_frames = _make_native_prefix_constraint(
                model,
                reference,
                condition_frames,
            )
            outputs.append(
                self.bundle.generate(
                    str(prompt),
                    num_frames,
    ...
    def constraints_from_json(self, path_or_data, *, device=None, dtype=torch.float32):
        ...
        return load_constraints_lst(
            ...
            self.skeleton,
            device=self._constraint_device(device),
            dtype=dtype,
        )
