...
class MoMaskPipeline(BasePipeline):
    ...
    def infer_motion_reconstruction(
        ...
        """Round-trip physical-scale HumanML3D-263 motions through the RVQ-VAE."""
        ...
        for index, motion in enumerate(motions):
            ...
            if usable < 4:
                raise ValueError("MoMask reconstruction requires at least 4 frames")
        ...
        for length, items in groups.items():
            for start in range(0, len(items), max(1, int(batch_size))):
                ...
                reconstructed, _, _ = bundle.vq_model((batch - mean) / std)
