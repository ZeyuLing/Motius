...
def _optional_points(
    ...
    *,
    ...
    if name not in payload:
        return None
    value = _numpy(payload[name], name=name)
    if value.ndim != 3 or value.shape[0] != frames or value.shape[-1] != 3:
    ...
    *,
    ...
    track = MonocularTrack(
        ...
        availability={
            ...
            "vertices": (
                "official_neutral_smpl"
                if vertices_camera is not None or vertices_world is not None
                else "not_exported"
            ),
            "pve": (
                "available_same_smpl_topology"
                if vertices_camera is not None or vertices_world is not None
                else "unavailable_without_exported_vertices"
            ),
