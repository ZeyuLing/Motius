...
class HyMotionT2MPipeline:
    ...
    def infer_text_to_motion(
        ...
        **batch_overrides,
        ...
        """Generate the artifact's native motion representation from text."""
        caption_list = [captions] if isinstance(captions, str) else list(captions)
        if isinstance(num_frames, int):
            frame_list = [int(num_frames)] * len(caption_list)
        else:
            frame_list = [int(value) for value in num_frames]
        if len(frame_list) != len(caption_list):
            raise ValueError("num_frames must contain one value per caption")
        batch = {
